		...
		return new Promise((resolve, reject) => {
			doesUsernameExist(email)
				.then(exists => {
					...
					Provider.create(values).exec((createErr, user) => {
		...
		return new Promise((resolve, reject) => {
			Provider
				...
				.exec(async (err, user) => {
					...
					console.log(user);					
				         const updatedUser = await Provider.updateOne({ email:user.email })
					.set({
...
const resetPasswordByResetToken = function (resetToken, newPassword) {
		return new Promise(async (resolve, reject) => {		
			var updatedUser = await Provider.updateOne({passwordResetToken:resetToken})
					.set({
...
const getIds = function (search, model) {
		return new Promise(async (resolve, reject) => {	
		        try{	
				let user = await Provider.find({where:{or:[{firstName: search}, {email:"search"}]}, select:["id"]});
